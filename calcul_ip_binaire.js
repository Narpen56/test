...
function display(type, _class, ip, box, multi = false) {
    if(maskClass[_class][0] != 0) {
        ...
        box.innerHTML += `<span class="white-text">
            <FONT color=white>
            <span class="result-key">Class:</span> ${_class}<br>
            <span class="result-key">Addresse Ip:</span> ${ip.join(".")}<br>
            <span class="result-key">Masque:</span> ${mask.join(".")}<br>
            <span class="result-key">Ip de réseau:</span> ${net_ip.join(".")}<br>
            <span class="result-key">Ip de Broadcast:</span> ${brd_ip.join(".")}<br>
            <span class="result-key">Première Ip:</span> ${first_ip.join(".")}<br>
            <span class="result-key">Dernière Ip:</span> ${last_ip.join(".")}<br>
            </FONT>
        </span>`
        if (multi && type == "hex") {
            box.innerHTML += '<button class="btn" onclick="back()">Retour</button>'
        } else if(!multi) {
            box.innerHTML += '<button class="btn" onclick="back()">Retour</button>'
        }
        return true
